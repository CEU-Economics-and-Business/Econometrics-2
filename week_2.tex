...
\begin{itemize}
	\item Non-causal: What is the average difference in earnings between people with $s+1$ and $s$ years of schooling?
...
\begin{eqnarray*}
\widehat{\tilde{\rho}}=\frac{Cov\left(Y_{i}, s_{i}\right)}{Var\left(s_{i}\right)}=\rho+\gamma^{'}\delta_{As}
\end{eqnarray*}
...
\begin{eqnarray*}
   \frac{Cov\left(Y_{i}, s_{i}\right)}{Var\left(s_{i}\right)} &=& \frac{Cov\left(\alpha+\rho s_{i}+ A_{i}^{'}\gamma+e_{i}, s_{i}\right)}{Var\left(s_{i}\right)}\\
    &=&\frac{\rho Var\left(s_{i}\right)+\gamma Cov\left(A_{i}, s_{i}\right)}{Var\left(s_{i}\right)}\\
    &=& \rho + \gamma\frac{Cov \left(A_{i}, s_{i}\right)}{Var\left(s_{i}\right)}\\
\end{eqnarray*}
...
\begin{frame}{An (almost) hypothetical case for discussion}
	\begin{itemize}
		\item The government of Zubrowka suspects that international university rankings (THE, QS, etc) do not reflect true qualities of domestic universities.
		\item The following study is commissioned:
		\begin{itemize}
			\item A random sample of college graduates working in Zubrowka are surveyed for their wages and last college attended
			\item The data is used to estimate
			\begin{equation*}
				\ln wage_i = \sum_c\alpha_cD_{ci} + \epsilon_i
			\end{equation*}
			where $D_{ci}=1$ if person $i$ graduated from college $c$.
			\item Colleges $c=1,\dots,C$ are ranked according to $\alpha_c$.
		\end{itemize}
		\item In this ranking, universities based in Zubrowka are quite competitive: the University of Lutz is ranked between MIT and Oxford.
	\end{itemize}
...
\begin{frame}{What's behind the data?}
	\begin{center}
