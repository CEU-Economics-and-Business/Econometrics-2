...
\item Examples:
       \begin{itemize}
          \item What would a given person earn if she obtained a different schooling level
          \item What would people earn, on average, if we could change their schooling in a perfectly controlled environment
\end{itemize}
\item Remember: in experiments causal variable is independent from potential outcomes
\end{itemize}
\end{frame}
...
\end{eqnarray*}
\begin{eqnarray*}
  \underbrace{E(Y_i|X_{i}, c_{i}=1)-E(Y_i|X_{i},c_{i}=0)}_{\texttt{observed difference |X}}=\underbrace{E(Y_{1i}-Y_{0i}|X_{i})}_{\texttt{difference in pot outcome |X}}
\end{eqnarray*}
...
\item Problem: $s_{i}$ may be correlated with potential outcomes $f_{i}\left(s\right)$, or $\eta_{i}$.
...
\frametitle{Bad Controls}
\begin{itemize}
\item Compare mean earnings for white color workers with or without college.

\begin{eqnarray*}
	Y_{i} &=& C_{i}Y_{1i}+ \left(1-C_{i}\right) Y_{0i} \\
  W_{i} &=& C_{i}W_{1i}+ \left(1-C_{i}\right) W_{0i}
\end{eqnarray*}
\begin{eqnarray*}
E\left[Y_{i}|c_{i}=1\right]-E\left[Y_{i}|c_{i}=0\right]&=& E\left[Y_{1i}-Y_{0i}\right]\\
E\left[W_{i}|c_{i}=1\right]-E\left[W_{i}|c_{i}=0\right]&=& E\left[W_{1i}-W_{0i}\right]
\end{eqnarray*}
\end{itemize}
