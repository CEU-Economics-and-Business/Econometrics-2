...
\begin{frame}
\frametitle{Omitted Variables Bias}
...
\begin{frame}
\frametitle{Measurement Error}
\textbf{Case 2}: Measurement Error in $X_i$

\begin{itemize}
    \item Consider a bivariate model without constant:
          \begin{eqnarray*}
	           Y_{i} &=&  \rho s^{*}_{i}+ u_{i} \\
    ...
    \item Classical Measurement Error (CME)
\begin{itemize}
\item $\epsilon_{i}\sim iid \left(0, \sigma^{2}_{\epsilon}\right)$
...
\frametitle{Estimated model}
 ...
 \begin{eqnarray*}
	Y_{i} &=&  \rho s^{*}_{i}+ u_{i} \\
	 ...
	 Y_{i}        &=&     \rho s_{i} + \tilde{u}_{i} \;\;\;\;\;\;\text{estimated model}
 \end{eqnarray*}
...
If $\rho>0$,  $Cov \left(s_{i},\tilde{u}_{i}\right) < 0$
...
\begin{eqnarray*}
     \tilde{\rho} &=&  \frac{Cov \left(Y_i,s_i\right)}{Var\left(s_{i}\right)}  \\
      ...
      &=&  \rho +  \frac{Cov \left( \tilde{u}_{i} ,s_i\right)}{Var\left(s_{i}\right)}  \\
...
\item $\lambda$ noise to signal ratio
    \item 1-$\lambda$ is interesting as it tells us that measurement error causes attenuation bias (this fraction is smaller than one)
\end{itemize}

      \begin{eqnarray*}
         1- \frac{\sigma^{2}_{\epsilon}}{\sigma^{2}_{\epsilon}+\sigma^{2}_{s^{*}}} =  \frac{\sigma^{2}_{s^{*}}}{\sigma^{2}_{\epsilon}+\sigma^{2}_{s^{*}}}=\frac{Var(s^{*})}{Var(s)}
      \end{eqnarray*}
...
Classical ME: $Cov\left(s^{*}_{i}, \epsilon_{i}\right)=0$, $Cov\left(X_{i}, \epsilon_{i}\right)=0$
 ...
 \textbf{Attenuation Bias}
\begin{eqnarray*}
\tilde{\rho} = \rho \frac{\sigma^2_{r^{*}_1}}{\sigma^2_{r^{*}_1}+\sigma^2_ \epsilon} 
\end{eqnarray*}

\begin {itemize}
\item  where ${r^{*}_1}$ is the population error from regression of $s^{*}_{i} = X_{i} \beta + {r^{*}_1}$
% regression of $s^{*}_{i}= X_{i}\beta + {r^{*}_1$
\item If $s_{i}$ is highly correlated with $X_i$, attenuation bias increases with inclusion of more covariates in the model
\end{itemize}
\end{frame}
...
\[\begin{array}{ccc}
   Cov\left(s^{*}_{i}, X_{i}\right)  \neq& 0 &\;\;\; \text{is high}\\
   Cov\left(\epsilon_{i}, X_{i}\right) =&0 &
   \end{array}
\]

Then:
\begin {itemize}
\item $X_{i}$'s that are correlated with $s^{*}_{i}$ soak up the signal in $s_{i}$, but leave the noise.
\item Because $X_{i}$ is uncorrelated with $\epsilon_{i}$ this exacerbates the ME problem.
\item Example:

$X_{i}$ parental education, earnings etc.
...
\begin{frame}
\frametitle{How big is the measurement error?}
...
\begin {itemize}
    ...
    \item $X_{i}$, variables varying by family
    \item $Z_{1i}, Z_{2i}$ variables specific to each twin
    \item $s_{1i}, s_{2i}$ schooling


\end {itemize}
 \begin{eqnarray*}
     Y_{1i} &=& \alpha+ \rho s_{1i}+ Z^{'}_{1i}\delta+ X^{'}_{i}\beta+u_{1i}+ A^{'}_{i}\gamma\\
     Y_{2i} &=& \alpha+ \rho s_{2i}+ Z^{'}_{2i}\delta+ X^{'}_{i}\beta+u_{2i}+ A^{'}_{i}\gamma
\end{eqnarray*}
\item Difference,
\begin{eqnarray*}
     Y_{1i} -Y_{2i}=  \rho \left(s_{1i}-s_{2i}\right)+ {\left(Z_{1i}-Z_{2i}\right)}^{'}\delta +u_{1i}-u_{2i}
     \end{eqnarray*}
...
\item Differencing takes out a lot of the signal, but not the noise. Especially, if correlation between $\left(s^{*}_{i2}, s^{*}_{i2}\right)$ is high
...
\begin{itemize}
\item To reduce measurement error, use average over multiple education reports $\frac{s^{1}_{1}- s^{2}_{2}}{2}+ \frac{s^{2}_{1}-s^{1}_{2}}{2}$ in first differenced model
